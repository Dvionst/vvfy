...
$(document).ready(function() {
	...
	<?php } ?>
		...
		$(document).on('mousemove', '.table-views-rows', function(e) {
			// vid = $(this).attr("vid");
			// alert(vid);
		});
		...
		$(document).on('change', '.select-user-tambah', function(e) {
			...
			$.ajax({
				...
				success:function(data){
					if (data=='no')
						$().toastmessage('showErrorToast', ' the team already exist ');
		...
		$(document).on('click', '#wrapper-conversation .close-conv', function(e) {
			// alert('123');
			$('#wrapper-conversation').fadeOut();
		...
		$(document).on('click', '.img-del', function(e) {
			...
			if (confirm){
				$.ajax({
					type:'GET',
					url: '<?php echo Yii::app()->createUrl("Project/delete"); ?>',
					data:"id="+pk,
					beforeSend: function() {
					  $("#loader").show();
					},
					success:function(data){
						...
						if (data=="sukses"){
						  $("#loader").fadeOut();
						  $('#barisbaru .baris').eq(index).remove();
						}else{
							alert(data);
		...
		function changeProjectUser(){
			 var data = [];
			 var due_date;
			 $( ".dash-proj-name" ).each(function(index,value) {
			 	// alert(index);
			 	due_date =  $('.dash-proj-name').eq(index).parents(".size-15").find('.dash-editabble-date input').val();
			 	 // $("span").parents("div")
			 	// alert($(this).val());
			 	data.push({
			 		"worker" : $(this).attr("worker"),
			 		"project_id" : $(this).val(),
			 		"manager":$(this).attr("manager"),
			 		"due_date":due_date
			 	});
			 });
			 // alert(JSON.stringify(data));
	...
	//		=	// 	dataType:'html'
		...
		$(document).on('click', '.img-upload,.img-folder', function(e) {
			...
			// alert(vid);
			$('#form-progres').show();	
			reloadListComment(pk,1,vid);		
			$('#full-screen').fadeIn();
		});

		$(document).on('click', '.img-feedback', function(e) {
			$('#full-screen').hide();
			var pk = $('#primary').val();
			var vid = $(this).attr("vid");
			reloadConversation(vid);
			$('#wrapper-conversation').fadeIn();
		});
		...
		function reloadListComment(project_id,tp,views_id){
			$.ajax({
				type: 'GET',
				url: '<?php echo Yii::app()->createAbsoluteUrl("Project/listcount"); ?>',
				data:"project_id="+project_id+"&type="+tp+"&view_id="+views_id,
				beforeSend: function() {
				...
				$.ajax({
					...
					success:function(data){
					  // $(".hidemodal").show();
						...
						// alert(data)	;
						if (data=='sukses'){
							$().toastmessage('showSuccessToast', 'progres has been sent');
							$('.hidemodal').click();
							$('.viewname').val("");
						...
						// reloadListComment($('#primary').val(),1,vid);
